...
class Workspace(Base):
    ...
    language_code: Mapped[str] = mapped_column(String(8), default="es")
...
class Channel(Base):
    ...
    autocomplete_text_plain: Mapped[str | None] = mapped_column(Text)
    autocomplete_entities_json: Mapped[str | None] = mapped_column(Text)
    signature_enabled: Mapped[bool] = mapped_column(Boolean, default=False)
    signature_text: Mapped[str | None] = mapped_column(Text)
    signature_text_plain: Mapped[str | None] = mapped_column(Text)
    signature_entities_json: Mapped[str | None] = mapped_column(Text)
    join_name_filter_enabled: Mapped[bool] = mapped_column(Boolean, default=False)
    join_approval_mode: Mapped[str] = mapped_column(String(16), default="manual")
    join_approval_interval_hours: Mapped[int | None] = mapped_column(Integer)
    join_approval_next_run_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), index=True
    )
    join_approval_last_run_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    ...
    autocomplete_buttons: Mapped[list["AutocompleteButton"]] = relationship(
        cascade="all, delete-orphan", lazy="selectin"
    )
...
class AutocompleteButton(Base):
    __tablename__ = "autocomplete_buttons"
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    channel_id: Mapped[int] = mapped_column(
        ForeignKey("channels.telegram_chat_id", ondelete="CASCADE"), index=True
    )
...
class RelayRule(Base):
    ...
    enabled: Mapped[bool] = mapped_column(Boolean, default=True)
    preserve_forward_header: Mapped[bool] = mapped_column(Boolean, default=False)
...
class Publication(Base):
    ...
    source_text_plain: Mapped[str | None] = mapped_column(Text)
    source_entities_json: Mapped[str | None] = mapped_column(Text)
...
class JoinRequestEvent(Base):
    ...
    approval_claimed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    approval_attempts: Mapped[int] = mapped_column(Integer, default=0)
    approval_error: Mapped[str | None] = mapped_column(Text)
